...
- **Encrypt any file** — produces a `<name>.cr` file next to the source (the *full* name plus `.cr`, so `a.bin` → `a.bin.cr`), embedding the original file name. *Advantage:* decryption restores the exact original name and bytes, nothing to remember, and two files whose names differ only by extension can't collide. Earlier versions stripped the extension, so `a.bin` and `a.copy` in one directory both became `a.cr` and the second silently destroyed the first — under `-d` that lost the original too. Existing `.cr` files still decrypt: the name comes from the encrypted header, never from the `.cr`'s own name.
...
**Keyboard:** with nothing picked, **Enter** opens the active tab's Browse dialog. In the password fields Enter moves from the first encrypting password to the second, and from the last field of either tab it starts the run (only once files are picked). **Tab** follows the same path — password(s), then the run button, which Enter presses. **Left/Right** flip between the Encrypt, Decrypt and About tabs from anywhere in the window (the arrows still move the caret inside a password field, and the tabs are frozen mid-run). In the Browse dialog the focus starts on the file list, so **up/down** walk the files and Enter picks them. The progress bar clears back to empty when a run ends.
...
Short flags bundle the usual way, so `-rdy` means `-r -d -y`. The long forms (`--recursive`, `--delete`, `--yes`) must be given separately.
